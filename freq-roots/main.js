...
function showResults(num, results, words) {
	...
	for (var i in results) {
		...
		for (j in item) {
			...
			t += "<td><a target='_blank' href='http://tanzil.ir/#" + wo.sure + ":" + wo.aye + "'>"+
				...
				wo.aye + "</a> " +
				wo.word + "" ;
			t += "<span class='r-"+wo.sure+"-"+wo.aye+" aye-text' class='aye-text'></span>";
			toBeLoadedAyes.push(wo);
	...
	for (var i=0; i<toBeLoadedAyes.length; i++) {
		var wo = toBeLoadedAyes[i];
		$.getJSON("aye.php?sure="+wo.sure+"&aye="+wo.aye+"&word="+wo.word)
		.done(function(json){
			e = $(".r-"+json.sure+"-"+json.aye);
			var pos = 0;
			for (var i=0; i<json.word-1; i++)
				pos = json.text.indexOf(' ', pos)+1;
			var e_pos = json.text.indexOf(' ', pos) == -1 ? json.text.length : json.text.indexOf(' ', pos);
			t = json.text.substr(0, pos) + '<span class="selected-word">' + json.text.substr(pos, e_pos - pos) + "</span>" + json.text.substr(e_pos);
			e.html(t);
			console.log('Aye ', json.text, e, pos, e_pos);
		})
		.fail(function (o, status, error) {
			console.log('error getting aye', error);
		});
	}
}

function search(wCount, q, words) {
		$.getJSON("q.php?num="+wCount+"&q="+q)
		.done(function(json){
